...
async function crearUsuario() {
    try {
        const res = await fetch('https://playground.4geeks.com/todo/users/Truficop', {
            method: "POST",
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ name: 'Truficop' })
        });
...
async function leerUsuario() {
    try {
        const resultado = await fetch('https://playground.4geeks.com/todo/users/Truficop');
...
async function leerUsuarios() {
    try {
        const res = await fetch('https://playground.4geeks.com/todo/users?offset=0&limit=100');
        const data = await res.json();
        console.log(data);
    } catch (error) {
        console.log('Error ==> ', error);
...
const Home = () => {
    ...
    async function crearToDo(item) {
        try {
            const res = await fetch('https://playground.4geeks.com/todo/todos/Truficop', {
                method: 'POST',
